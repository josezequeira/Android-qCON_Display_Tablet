...
/* Start main class */
public class BSRBoxView extends View {
	...
	/** ---- Class constructors ---- **/
	public BSRBoxView(Context context) {
		super(context);
	...
	public BSRBoxView(Context context, AttributeSet attrs) {
		super(context, attrs);
        ...
        //
        // This call uses R.styleable.BSRBoxView, which is an array of
        // the custom attributes that were declared in values/attrs.xml.
		TypedArray attributes = context.getTheme().obtainStyledAttributes(
				attrs, R.styleable.BSRBoxView, 0, 0);
		
		try {
			// Retrieve the values from the TypedArray and store into
	        // fields of this class.
			lb_Background_Size = attributes.getInteger(R.styleable.BSRBoxView_BSR_label_Background_Size, 25);
			lb_Background_Color = attributes.getColor(R.styleable.BSRBoxView_BSR_label_Background_Color, Color.WHITE);
			lb_text = attributes.getString(R.styleable.BSRBoxView_BSR_label_Text);
			lb_Text_Color = attributes.getColor(R.styleable.BSRBoxView_BSR_label_Text_Color, Color.BLACK);
			lb_Text_BelowBaseLine = attributes.getBoolean(R.styleable.BSRBoxView_BSR_label_Text_BelowBaseLine, false);
			vl_text = attributes.getString(R.styleable.BSRBoxView_BSR_value_Text);
			vl_Text_Color = attributes.getColor(R.styleable.BSRBoxView_BSR_value_Text_Color, Color.WHITE);
			vl_Text_BelowBaseLine = attributes.getBoolean(R.styleable.BSRBoxView_BSR_value_Text_BelowBaseLine, false);
		} finally {
	...
	protected void onDraw(Canvas canvas) {
		...
		// Calculate value background height
		vl_Height = mViewHeight - lb_Height;
		
		// Calculate labels and values xAxis point
		text_xAxisPoint = mViewWidth/2;
		// Calculate labels and value yAxis point
		lb_yAxisPoint = lb_Height/2;
		vl_yAxisPoint = lb_Height + vl_Height/2;
		...
		// Change text size until match view width
		while (textBoundaries.width() > mViewWidth-10) {
