...
public class Srv_USB_ConnectionSynchronization extends Service {
	
	/*** Start variables ***/
	// Debugging
	private static final String TAG = "Srv_USB_ConnectionSynchronization";
	...
	private static D2xxManager ftdi_Manager;
	...
	public void onCreate() {
		...
		// Get FTDI manager instance
		try {
			ftdi_Manager = D2xxManager.getInstance(this);
	...
	public int onStartCommand(Intent intent, int flags, int startId) {
		...
		if (ftdi_Manager != null) {
			...
			devCount = ftdi_Manager.createDeviceInfoList(this);
			
			// Try to open device
			if (devCount > 0) {
				ftDev = ftdi_Manager.openByIndex(this, 0);
				...
				if (ftDev.isOpen() == true) {
					...
					// Discards any data form the specified driver buffer
					ftDev.purge((byte) (D2xxManager.FT_PURGE_TX));
					// Restarts the driver's IN thread
					ftDev.restartInTask();
					...
					// Start read thread
					startReadDataThread(connProtocol);
					// Start receiver
					startReceiver();
	...
	/** --------------------- Stuff to read data from device --------------------- **/
	/* ---- Create and start thread to read data from device ---- */
	private void startReadDataThread(boolean protFlag) {
		// Create read thread
		readDevice = new ReadThread("Thr_ReadUSBdevice", fifoBufferToSynch, protFlag);
		// Start thread
		readDevice.start();
	}
	...
	private class ReadThread extends Thread {
		...
		/** --- Thread constructor --- **/
		public ReadThread(String ThreadName, Class_CircularBuffer fifoBufferSynch, boolean protFlag) {
			// Set threadName
			super(ThreadName);
			// Write on log
		...
		public void run() {
			while (threadRunning == true) {
				// Read from FTDI Chip device
				synchronized(ftDev) {
					// Get number of bytes available to read on the device
					bytesToRead = ftDev.getQueueStatus();
					...
					if (bytesToRead > 0) {
						...
						// Read from FTDI chip device
						ftDev.read(readBuffer, bytesToRead);
	...
	private class SynchronizeDataThread extends Thread {
		...
		// ---- Extract synchronized data by protocol and prepare it to broadcast ----\\
